...
class mainApp1(QMainWindow, FORM_CLASS):
    ...
    def handle_ui(self):
        ...
        self.setWindowIcon(QIcon('photos/elogo.png'))
    ...
    def select_operation_show(self):
        self.label_17.setText('DECRYPT')

    def select_operation_hide(self):
        self.label_17.setText('ENCRYPT')

    def setOutput(self):
        url_output = QFileDialog.getSaveFileName(self, 'Save As', '', 'Text Files (*.txt);; Image Files (*.png)')
    ...
    def process(self):
        ...
        if choice == 'ENCRYPT':
            if (key_key != '') and (message != '') and (image != '') and (pathSave != ''):
                try:
                    QMessageBox.about(self,'Please Wait', 'Waiting ...!')
                    ...
                    QMessageBox.about(self, 'Error', 'Failed operation, Try again!')
            else:
                QMessageBox.about(self, 'Error', 'Please Enter all fields to continue...')
        elif choice == 'DECRYPT':
            if (key_key != '') and (image != ''):
                try:
                    ...
                    if op == True:
                        ...
                        open_open.close()
                        time.sleep(1)
                        QMessageBox.about(self, 'Success', 'Your message was stored as a text file:\n{}'.format(file_secure_output))
                        self.plainTextEdit.setPlainText('')
                        self.plainTextEdit_2.setPlainText('')
                        self.lineEdit_2.setText('')
                        self.lineEdit.setText('')
